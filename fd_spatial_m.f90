...
!
!       This version is for Ted's mixing layer profiles
!
!       Revised: 2-9-96
!=============================================================================!
        use stuff
...
!.... make a mixing layer mesh

        call mgengrid( y, eta, deta, d2eta, vm )

!.... read Ted's mean profile

        call mgetmean( vm, y, eta)
...
!.... C0
        ...
        i = 1
        i0 = (i-1)*ndof
        idof = 1                 ! continuity at the bottom boundary
        ...
        i = ny
        i0 = (i-1)*ndof
        idof = 1                       ! continuity at the top boundary
...
#else
        call ZGETRF_(ndof*ny, ndof*ny, C0, ndof*ny, ipvt, info)
...
#else
        call ZGETRS_('N', ndof*ny, ndof*ny, C0, ndof*ny, &
                    ipvt, C1, ndof*ny, info)
        if (info.ne.0) then
          write(*,*) 'ZGETRS: ',info
          call exit(1)
...
#else
        if (ievec.eq.1) then
          call ZGEEV_('N', 'V', 2*ndof*ny, B0, 2*ndof*ny, alp, evec, &
                      2*ndof*ny, evec, 2*ndof*ny, work, lwork, rwork, info)
        else
          call ZGEEV_('N', 'N', 2*ndof*ny, B0, 2*ndof*ny, alp, evec, &
                      2*ndof*ny, evec, 2*ndof*ny, work, lwork, rwork, info)
        end if
#endif
        if (info.ne.0) then
